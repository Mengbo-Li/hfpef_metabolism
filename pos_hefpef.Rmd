---
title: "Lipid (pos) uptake/release in HFpEF and healthy groups"
author: "Mengbo Li"
date: "`r Sys.Date()`"
output: 
   html_document:
...
# cleaned data
load(here("data/230314-pos_lipid_data_for_HFpEF_QCed.rda"))
```
...
```{r}
# table(colnames(pos) == meta$sample_name)
...
ggvenn(list(Metab = unique(meta_metab$subject), 
            POS = unique(meta$subject))) +
  ggtitle("Subject overlap")
```

Exact same cohort as metabolite data. 
...
#| fig-width: 5
#| fig-height: 4
RLA(t(pos[, meta$sample_name[order(meta$run_order)]]), 
...
mds <- ggMDS(pos, meta, "sample_name")
...
<!-- ```{r} -->
<!-- # outliers -->
<!-- outliers <- filter(mds, y < -0.5) |>  -->
<!--   select(sample_name) -->
<!-- outliers -->

<!-- meta_filt <- filter(meta, !(sample_name %in% outliers$sample_name)) -->
<!-- dim(meta) -->
<!-- dim(meta_filt) -->
<!-- pos_filt <- pos[, meta_filt$sample_name] -->
<!-- dim(pos_filt) -->

<!-- mds_filt <- ggMDS(pos_filt, meta_filt, "sample_name") -->

<!-- ggplot(mds_filt, aes(x = x, y = y, color = grouping, shape = sampling_site)) +  -->
<!--   geom_point(size = 3, alpha = 1, stroke = 1) +  -->
<!--   theme_mengbo() +  -->
<!--   scale_shape_manual(values = c(16, 8)) +  -->
<!--   scale_color_manual(values = cond.col) +  -->
...
```{r}
dupcor <- duplicateCorrelation(pos, design, block = meta$subject)
fit <- lmFit(pos, design, block = meta$subject, correlation = dupcor$consensus.correlation)
...
# topTable(efit, coef = "usage_healthy", number = Inf) |>
#   mutate(FC = unlog2FC(logFC), .after = 1) |>
#   write.csv(file = "tables/pos/230315-pos_lipids_FigXXXX_DA_CSminusART_HEALTHY.csv")
```
...
# volcano plot
efit$genes <- data.frame(feature = rownames(pos)) |> 
...
#   select(-Symbol) |>
#   mutate(FC = unlog2FC(logFC), .after = 1) |>
#   write.csv(file = "tables/pos/230315-pos_lipids_FigXXXX_DA_CSminusART_HFPEF.csv")
```
...
compareLogFC(efit, 
             ...
             extra.labels = c("TG(16:0_14:0_22:6)"),
...
pos_art <- pos[, meta$sample_name[meta$sampling_site == "ART"]]
ARTameans <- data.frame(feature = rownames(pos_art), grpAmean = rowMeans(pos_art, na.rm = TRUE))
...
# topTable(efit, coef = "usage_healthy_female", number = Inf) |>
#   mutate(FC = unlog2FC(logFC), .after = 1) |>
#   write.csv(file = "tables/pos/230315-pos_lipids_FigXXXX_DA_CSminusART_HEALTHYandFEMALE.csv")
```
...
# topTable(efit, coef = "usage_healthy_male", number = Inf) |>
#   mutate(FC = unlog2FC(logFC), .after = 1) |>
#   write.csv(file = "tables/pos/230315-pos_lipids_FigXXXX_DA_CSminusART_HEALTHYandMALE.csv")
```
...
# topTable(efit, coef = "usage_HFPEF_female", number = Inf) |>
#   mutate(FC = unlog2FC(logFC), .after = 1) |>
#   write.csv(file = "tables/pos/230315-pos_lipids_FigXXXX_DA_CSminusART_HFpEFandFEMALE.csv")
```
...
# topTable(efit, coef = "usage_HFPEF_male", number = Inf) |>
#   mutate(FC = unlog2FC(logFC), .after = 1) |>
#   write.csv(file = "tables/pos/230315-pos_lipids_FigXXXX_DA_CSminusART_HFpEFandMALE.csv")
```
...
```{r}
da_pos_lipids <- rownames(efit)[decideTests(efit)[, "usage_HFPEF_male"] != 0]
da_pos <- filter(pos_lipids, clean_final %in% da_pos_lipids) |> 
  mutate(Direction = as.factor(decideTests(efit)[da_pos_lipids, "usage_HFPEF_male"]), .after = 1)
levels(da_pos$Direction) <- c("Uptake", "Release")
table(da_pos$Direction)
table(da_pos$class)

# pie chart
ggplot(da_pos, aes(x = 0, fill = class))+
  geom_bar(width = 1) +
...
```{r}
da_pos_tile <- da_pos |> 
  select(class, clean_final, Direction, fa1, fa2, fa3) |> 
  pivot_longer(4:6) |> 
  filter(value != "") |> 
  select(-name) |> 
...
# release
da_hf_male_release <- filter(da_pos_tile, Direction == "Release") |> 
  group_by(class, value) |> 
  mutate(no_fa = as.character(length(value))) |> 
  ggplot(aes(x = Carbon, y = `Double bonds`, fill = no_fa)) +
  geom_tile(color = "white", linewidth = 0.2, alpha = 0.8, show.legend = FALSE) + 
  scale_fill_manual(values = ggsci::rgb_material("red")[c(3, 7)]) + 
  geom_text(aes(label = no_fa), size = 5, vjust = 0.5, color = "black") +
  facet_wrap(~ class, nrow = 1) + 
  scale_x_continuous(limits = c(14.5, 24.5), breaks = 15:24) + 
...
# uptake
da_hf_male_uptake <- filter(da_pos_tile, Direction == "Uptake") |> 
  group_by(class, value) |> 
  mutate(no_fa = as.character(length(value))) |> 
  ggplot(aes(x = Carbon, y = `Double bonds`, fill = no_fa)) +
  geom_tile(color = "white", linewidth = 0.2, alpha = 0.8, show.legend = FALSE) + 
  scale_fill_manual(values = ggsci::rgb_material("blue")[c(2, 4, 10)]) + 
  geom_text(aes(label = no_fa), size = 5, vjust = 0.5, color = "black") +
  facet_wrap(~ class, nrow = 1) + 
  scale_x_continuous(limits = c(5.5, 29.5), breaks = 6:29) +
...
da_pos_release <- filter(da_pos_tile, Direction == "Release") |>
  group_by(class, value) |> 
...
da_pos_uptake <- filter(da_pos_tile, Direction == "Uptake") |>
  group_by(class, value) |> 
  mutate(fa_occurance = as.character(length(value))) |> 
  select(-clean_final) |> 
  distinct() |> 
...
da_pos_pie <- add_row(da_pos_release, da_pos_uptake) |> 
  ...
  mutate(radius = fa_occurance_total/10 + 0.1)

table(da_pos_pie$radius)

ggplot() + 
  geom_scatterpie(data = da_pos_pie, aes(x = Carbon, y = `Double bonds`), 
  ...
  scale_y_continuous(limits = c(-0.5, 5.5), breaks = 0:5) + 
  theme_mengbo(clean_background = FALSE) + 
  ggtitle("Significant DA usage pos lipids in HFpEF male") + 
...
ggplot() + 
  geom_scatterpie(data = da_pos_pie, aes(x = Carbon, y = `Double bonds`, r = radius), 
                  cols = c("fa_occurance_uptake", "fa_occurance_release"), 
                  pie_scale = 0.8, 
                  legend_name = "Direction") + 
  facet_wrap(~ class, nrow = 1) +
  ...
  ggtitle("Significant DA usage pos lipids in HFpEF male") + 
  theme(axis.text = element_text(size = 12)) +
  geom_scatterpie_legend(radius = da_pos_pie$radius, n = 5, x = 7, y = 4, 
                         labeller = function(x) {x = sort(unique(da_pos_pie$fa_occurance_total))}) + 
  labs(x = "Carbons", y = "Double bonds")
```
...
vol_usage_HFmale <- plotVolcano(efit, 
                                ...
                                label.P.neg = 0.03, lable.P.pos = 0.03) + 
  ggtitle("HFpEF - Male")
...
rank_hf <- ggparcoord(sex_hf, columns = c(5, 10), 
                      ...
                      showPoints = TRUE) + 
  scale_y_reverse() + 
  scale_color_manual(values = c(sex.col, "#F2F2F2"), 
                     labels = c("Only female", "Only male", "Neither")) + 
  theme_mengbo(clean_background = FALSE) + 
  scale_x_discrete(labels = c("Female", "Male")) + 
  labs(title = "HFpEF", 
       x = "", 
...
usage <- lapply(subjs, function(ss) { 
  ...
  res <- pos[, cs_smp] - pos[, art_smp]
}) %>% do.call(cbind, .)
colnames(usage) <- subjs
rownames(usage) <- rownames(pos)
...
#        metabolite %in% c(unique(metabolite)[1:9], "SM(d32:4)+H")) |> 
...
```{r}
clin2 <- plot_MTBvsCLIN("Hex1Cer(d18:1_25:0)", "ex_pas") + 
  xlab("log2ex_pas") + 
  labs(subtitle = "In HFpEF")

clin3 <- plot_MTBvsCLIN("TG(16:1_22:3_24:2)", "ex_pas") + 
  xlab("log2ex_pas")  + 
  labs(subtitle = "In HFpEF")

ggarrange(clin2, clin3, nrow = 1, ncol = 2)
```

```{r}
## hfpef results
cont_ttest <- lapply(unique(sigRes$var[sigRes$stratum == "HFpEF"]), function(ii) {
  tmp <- select(usage_meta, grouping, all_of(ii), subject) |> 
    distinct() |> 
    rename(var = 2) |> 
    filter(!is.na(var)) |> 
  ...
  #   rename(coef = logFC) |>
  #   write.csv(file = paste("tables/pos/230315-pos_lipids_FigXXXX_usageAssociatedClinicalVariables_", ii, "_inHFpEF.csv", sep = ""))
...
```{r}
plot_MTBvsCLIN("LPC(18:4)", "rest_pam") + 
  xlab("log2rest_pam") + 
  labs(subtitle = "In healthy")
```


```{r}
# healhty results
cont_ttest <- lapply(unique(sigRes$var[sigRes$stratum == "healthy"]), function(ii) {
  tmp <- select(usage_meta, grouping, all_of(ii), subject) |> 
    distinct() |> 
    rename(var = 2) |> 
    filter(!is.na(var)) |> 
  ...
  # topTable(efit, number =  Inf, coef = "log2var") |>
  #   rename(coef = logFC) |>
  #   write.csv(file = paste("tables/neg/230315-pos_lipids_FigXXXX_usageAssociatedClinicalVariables_", ii, "_inHealthy.csv", sep = ""))
